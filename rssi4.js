        ...
        if (Array.isArray(payload)) {
            payload.forEach(item => {
                if (typeof item === 'object' && filterMacIds.includes(item.mac)) {
                    ...
                    const distance = calculateDistance(smoothedRssi); // Use smoothed RSSI for distance
                    ...
                    if (distanceSamples[item.mac].length === 10) {
                        ...
                        // Optional: Limit the size of latestProcessedData to avoid memory issues
                        if (latestProcessedData.length > 1000) {
                            latestProcessedData.shift(); // Remove the oldest data
                        }
                    }
                }
            });
